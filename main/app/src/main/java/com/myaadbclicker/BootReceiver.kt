...
class BootReceiver : BroadcastReceiver() {
    ...
    private fun handleBootCompleted(context: Context) {
        Log.d(TAG, "Sistem başlatma tamamlandı - Servisler başlatılıyor")
    ...
    private fun startServices(context: Context) {
        try {
            Log.d(TAG, "Servisler başlatılıyor...")
            ...
            Log.e(TAG, "Servis başlatma hatası: ${e.message}")
            ...
            Handler(Looper.getMainLooper()).postDelayed({
                try {
                    ...
                    Log.d(TAG, "Servis başlatma yeniden denendi")
                } catch (retryException: Exception) {
                    Log.e(TAG, "Yeniden deneme hatası: ${retryException.message}")
                }
            }, 5000L)
        }
    }
